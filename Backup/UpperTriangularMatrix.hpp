...
template <class T>
UpperTriangularMatrix<T>::UpperTriangularMatrix(const unsigned int order)
{
	m_order = order;
...
template <class T>
UpperTriangularMatrix<T>::UpperTriangularMatrix(const unsigned int order, const T value)
{
	m_order = order;
	m_size = (order*(order+1))/2;
	ptr_to_data = new T[m_size];
...
template <class T>
UpperTriangularMatrix<T>::UpperTriangularMatrix(const initializer_list< initializer_list<T> > list)
{
	m_order = list.size();
...
//////// Virtual functions ////////
template <class T>
T UpperTriangularMatrix<T>::operator()(const unsigned int row, const unsigned int col) const
{
...
template <class T>
T& UpperTriangularMatrix<T>::operator()(const unsigned int row, const unsigned int col)
{
	if(row >= m_order || col >= m_order)
	{
        throw Out_Of_Range_Err(m_order*row+col);
...
template <class T>
Matrix<T> UpperTriangularMatrix<T>::operator*(const Base_Matrix<T>& rhs) const
{
...
///////// Member functions ////////
template <class T>
UpperTriangularMatrix<T>& UpperTriangularMatrix<T>::operator=(const Base_Matrix<T>& rhs)
{
...
{
	UpperTriangularMatrix<T> neg(m_order);
...
template <class T>
UpperTriangularMatrix<T>  UpperTriangularMatrix<T>::operator*(const T scalar) const
{
	UpperTriangularMatrix<T> scaled(m_order);
...
template <class T>
UpperTriangularMatrix<T>& UpperTriangularMatrix<T>::operator*=(const T scalar)
{
    T* p = ptr_to_data + m_size;
...
template <class T>
Matrix<T> UpperTriangularMatrix<T>::Transpose() const
{
...
template <class T>
void UpperTriangularMatrix<T>::SetAll(T value)
{
...
template <class T>
void UpperTriangularMatrix<T>::Zero()
{
	for(unsigned int i = 0; i < m_size; i++)
	{
		ptr_to_data[i] = 0;
...
template <class T>
void UpperTriangularMatrix<T>::Resize(const unsigned int order)
{
	m_order = order;
	m_size = (order*(order+1))/2;
	if(ptr_to_data != NULL)
...
template <class T>
void UpperTriangularMatrix<T>::copy(const UpperTriangularMatrix<T>& src)
{
